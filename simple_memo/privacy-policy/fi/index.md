# Tietosuojakäytäntö — Simple Memo
...
Simple Memo (jäljempänä "sovellus") on mobiilisovellus muistiinpanojen luomiseen ja järjestämiseen. Tässä käytännössä selitetään, mitä tietoja sovelluksen käytöstä kerätään ja miten niitä käsitellään.

## 2. Sovelluksen keräämät tiedot

### Vain laitteellesi tallennettavat tiedot

Luomiesi muistiinpanojen sisältö ja otsikot sekä asetukset, kuten teeman (vaalea/tumma) valinta, tallennetaan vain laitteellesi. Näitä tietoja ei koskaan lähetetä ulkopuolelle, ja ne poistetaan, kun poistat sovelluksen asennuksen.
...
**Seurannasta iOS:ssä**: kohdennettujen mainosten toimittamiseksi sovellus voi pyytää App Tracking Transparency -lupaa esimerkiksi ensimmäisellä käynnistyskerralla. Jos myönnät luvan, IDFA:ta voidaan käyttää seuraamaan sinua muiden yritysten sovelluksissa tai verkkosivustoilla. Luvan myöntäminen on vapaaehtoista, ja sovellus toimii normaalisti, vaikka kieltäytyisit — näytettävät mainokset eivät tällöin vain ole kohdennettuja.
...
Kerättyjä tietoja käyttää Google mainosten toimittamiseen (mukaan lukien kohdentaminen luvan myöntämisen tapauksessa) ja petosten estämiseen. Kehittäjä ei käytä näitä tietoja käyttöanalyysiin tai muihin tarkoituksiin kuin mainontaan.
...
- **iOS**: valitse "Pyydä sovellusta olemaan seuraamatta" ensimmäisessä seurantaluvan kyselyssä, tai siirry myöhemmin kohtaan Asetukset → Tietosuoja ja turvallisuus → Applen mainokset → poista käytöstä "Kohdennetut mainokset"
