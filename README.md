...
<slug>/
  ...
  support/               # サポートページ。**審査で必須**（忘れやすい）
    index.md
  terms-of-service/      # 必要なアプリだけ追加（同様に言語ごとのサブディレクトリ）
    index.md
```
...
**`terms-of-service/`は全アプリ必須ではない。** オンライン機能／利用者間のやり取り／
利用制限を課す可能性／ゲーム内通貨のいずれかがあるアプリにだけ用意する
（`~/AIFiles` Issue #17）。2026-08-22時点でAppFactoryの配信対象アプリを全数調査した結果、
「オンライン機能／利用者間のやり取り」「利用制限」に該当するものは無し（アカウント機能
自体が存在しないため）。該当するのは「ゲーム内通貨があるか」のみで、対象は
`crowd_rush`・`once_dungeon`（ともにIAP紐付けの無い、ゲーム内でのみ完結する通貨）。
`stone_knights`はオンライン対戦があるため別の理由で必要。それ以外のアプリ
（電卓・タイマー等のユーティリティ、通貨の無いゲーム）には不要 —
`privacy-policy/`と`support/`だけで足りる。開発中で未配信のアプリ
（`deep_delve`・`lamp_road`等、通貨はあるが`distribution: in-development`）は
配信開始のタイミングで判断し直す。
